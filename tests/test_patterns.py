...
def wave_candles(
    ...
    *,
    ...
    closes.append(swings[-1])
    out: list[Candle] = []
    prev = closes[0]
    for i, close in enumerate(closes):
        out.append(_candle(i, close, timeframe=timeframe, open_=prev, t0=t0, volume=volume + i))
        prev = close
    return out
...
def test_unfinished_and_future_pivots_cannot_create_hypothesis_at_t():
    xs = _continuation()
    last_known = confirmed_pivots(xs, 3, 3)[-1]
    t = last_known.known_at
    future = Pivot(
        index=99,
        known_at_index=102,
    ...
    hyps = hypotheses_at(xs + [unfinished], t, timeframe="4h", pivots=[*confirmed_pivots(xs, 3, 3), future])
    blob = freeze_hypotheses(hyps)
    assert "99.0" not in str(blob)
    assert all("99.0" not in item.detail for h in hyps for item in h.evidence)
...
def test_future_pivot_cannot_confirm_at_t():
    ...
    cont = next(h for h in hyps if h.kind == "continuation" and h.direction == "bear")
    ...
    future_low = Pivot(
        ...
        price=cont.confirmation_rules[1].price - 1.0,
        kind="low",
    )
    still = evaluate_hypothesis(cont, xs, t, pivots=[future_low])
    assert still.status in {"candidate", "active"}
    assert still.status != "confirmed"
    later = t + timedelta(hours=20)
    confirmed = evaluate_hypothesis(cont, xs, later, pivots=[future_low])
...
def test_unfinished_candle_cannot_confirm():
    xs = _continuation()
    t = _as_of(xs[-1])
    cont = next(h for h in hypotheses_at(xs, t, timeframe="4h") if h.kind == "continuation")
    level = next(r.price for r in cont.confirmation_rules if r.kind == "close_below")
    unfinished = _candle(
        len(xs),
        level - 0.5,
        timeframe="4h",
    ...
    still = evaluate_hypothesis(cont, xs + [unfinished], candle_period_end(unfinished) if unfinished.is_closed else t + timedelta(hours=4))
    # period_end of an unfinished bar is ignored because is_closed is false
    still = evaluate_hypothesis(cont, xs + [unfinished], t + timedelta(hours=8))
    assert still.status in {"candidate", "active"}
    assert still.status != "confirmed"


def test_perturb_candles_after_t_leaves_hypotheses_unchanged():
    xs = _continuation() + wave_candles([7.0, 6.5, 7.2], spacing=8, timeframe="4h", t0=T0 + timedelta(days=20))
    cut = 40
    t = _as_of(xs[cut])
    before = freeze_hypotheses(hypotheses_at(xs, t, timeframe="4h"))
    mutated = list(xs)
...
def test_invalidation_cannot_be_moved():
    xs = _continuation()
    t = _as_of(xs[-1])
    hyp = next(h for h in hypotheses_at(xs, t, timeframe="4h") if h.kind == "continuation")
...
def test_five_minute_relief_does_not_confirm_four_hour_continuation():
    four_h = _continuation("4h")
    t = _as_of(four_h[-1])
    cont = next(
        h
        for h in hypotheses_at(four_h, t, timeframe="4h", parent_regime="bearish")
        if h.kind == "continuation" and h.direction == "bear"
    )
    assert cont.status in {"candidate", "active"}
    assert cont.regime_relation == "aligned"
    confirm_price = next(r.price for r in cont.confirmation_rules if r.kind == "close_below")
    invalid_price = cont.invalidation[0].price
    relief: list[Candle] = []
    price = four_h[-1].close
    for i in range(12):
        price = price + 0.08
        relief.append(
            _candle(
    ...
    # Also include a 5m close that would satisfy the 4H close_below if TF were ignored.
    relief.append(
        _candle(
            12,
            confirm_price - 0.25,
    ...
    hold = _candle(
        ...
        confirm_price - 0.25,
        timeframe="4h",
        open_=four_h[-1].close,
        t0=four_h[0].open_time,
    )
    confirmed = evaluate_hypothesis(cont, four_h + [hold], candle_period_end(hold))
    assert confirmed.status == "confirmed"
...
def test_elliott_is_candidate_and_never_privileged():
    ...
    if not elliott:
        pytest.skip("insufficient alternating pivots for an Elliott candidate on this fixture")
    for hyp in elliott:
        assert hyp.status == "candidate"
        assert hyp.privileged is False
        assert hyp.framework == "elliott"
...
def test_book_evaluate_does_not_move_invalidation():
    ...
    hold = _candle(
        len(xs),
        hyp.invalidation[0].price + 0.4,
        timeframe="4h",
        open_=xs[-1].close,
        t0=xs[0].open_time,
    )
    updated = book.evaluate(hyp.id, xs + [hold], candle_period_end(hold))
...
def test_required_fields_present():
    xs = _continuation()
    t = _as_of(xs[-1])
    hyp = next(iter(hypotheses_at(xs, t, timeframe="4h")))
    payload = hyp.to_dict()
    for key in (
        "evidence",
        "counter_evidence",
