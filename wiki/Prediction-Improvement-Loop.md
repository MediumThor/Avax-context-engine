...
- Live path journals empirical next-10 quantiles when history exists, otherwise honest `baseline.drift20`. Walk-forward Brier/ECE/coverage are scored, not promoted.
...
- Snapshot theses now attach competing bull/bear summaries with invalidation frozen at open. Zone lifecycle still does not drive `build_snapshot`.
