...
Research helper `walk_forward_quantiles` scores `freqai.quantiles.research.v1` q50 MAE and empirical q10–q90 coverage against `baseline.zero` and `baseline.drift20` on chronological origins only. It never sets a product promotion flag. Coverage is a hit rate, not ECE. `GET /api/v1/forecast/metrics?challenger=true` attaches that q50 MAE and `q50_mae_minus_drift20_mae` per horizon (step 80), plus HTF-gated drift MAE and `htf_mae_minus_drift20_mae` from `walk_forward_htf_regime` (min_history 200, step 40). The Accuracy dest requests that payload. `/market` metrics stay without the challenger walk-forward so the chart path stays fast. `promotion_allowed` stays false even if either helper is lower on some horizons. A fixture HTF win is not a sealed live-plan promotion.
