...
- `timeframes.*.swing_pivots` — last confirmed window extrema (`left=right=3`) with extreme `time` (unix open), `known_at`, `price`, and `kind`. A pivot is absent until its confirmation bar is in the visible closed series. Markers are structure, not confidence.
